...
bool video_reader_open(VideoReaderState* state, const char* filename) {
	...
	if (av_format_ctx) {
		printf("Allocated AVFormatContext\n");
	}
 ...
 bool video_reader_read_frame(VideoReaderState* state, uint8_t* frame_buffer, int64_t* pts) {
	...
	while (av_read_frame(av_format_ctx, av_packet) >= 0) {
		...
		AVPixelFormat pix_fmt = correct_for_deprecated_pixel_format(static_cast<AVPixelFormat>(av_frame->format));
		// Convert the frame to RGB24
		SwsContext* sws_ctx = sws_getContext(width, height, pix_fmt,
			width, height, AV_PIX_FMT_RGB24,
			SWS_BILINEAR, NULL, NULL, NULL);
		uint8_t* dest[4] = { frame_buffer, NULL, NULL, NULL };
		int dest_linesize[4] = { 3 * width, 0, 0, 0 };
		sws_scale(sws_ctx, av_frame->data, av_frame->linesize, 0, height, dest, dest_linesize);
